...
import id.kemenkes.sip.ukp.entity.UKP3Detail;

@Controller
@RequestMapping("/web/ukp3")
public class UKP3Controller {

    static final Logger LOGGER = LoggerFactory.getLogger(UKP3Controller.class);
    ...
    public String howTo(ModelMap model){
		...
		UKP3 ukp3 = new UKP3();
		ukp3.setSubmittedDate(null);
		ukp3.setKodePuskesmas("P140101102");
		ukp3.setNamaPuskesmas("BANGKINANG KOTA");
		ukp3.setProvinsi(14);
		ukp3.setKotaKabupaten(1401);
		ukp3.setBulan(1);
		ukp3.setTahun(2016);
		ukp3.setPoskesdes_lapor(5);
		ukp3.setPoskesdes_total(5);
		ukp3.setPustu_lapor(5);
		ukp3.setPustu_total(5);
		
		List<UKP3Detail> details = new ArrayList<UKP3Detail>();
		UKP3Detail detail1 = new UKP3Detail();
		detail1.setNomor(1);
		detail1.setJenis_penyakit("Penyakit 01");
		detail1.setIcdx("ICDX 01");
		details.add(detail1);
		
		UKP3Detail detail2 = new UKP3Detail();
		detail2.setNomor(2);
		detail2.setJenis_penyakit("Penyakit 02");
		detail2.setIcdx("ICDX 02");
		details.add(detail2);
		
		ukp3.setDetail(details);
		
		model.addAttribute("responseJsonFormat", "{\"status\": \"success\", \"messageCode\": \"\",\"message\": \"UKP3-9B43BNTO\"}");
		model.addAttribute("requestJsonFormat", new Gson().toJson(ukp3));
        model.addAttribute("menu", "howto.ukp3");
    ...
    public String viewTenant(ModelMap model, @PathVariable("id") String id){
        ...
        UKP3 ukp3 = ukp3Repository.getById(id);
        
        model.addAttribute("ukp3", ukp3);
        model.addAttribute("menu", "report.ukp3");
    ...
    public String list(ModelMap model, HttpServletRequest request, @RequestParam(value = "kodePuskesmas", required = false) String kodePuskesmas, 
        ...
        Page<UKP3> returnPage = ukp3Repository.getByFilter(kodePuskesmas, tahun, bulan, page);
	...
	public String deleteUKP3(ModelMap model, RedirectAttributes redirectAttributes,
			@RequestParam(value = "id", required = false) String[] ids) {
		boolean allDeleted = true;
		if (ids != null && ids.length > 0) {
			for (String id : ids) {
				try {
					ukp3Repository.deleteById(id);;
				} catch (DataIntegrityViolationException e) {
					LOGGER.error("Error in deleting ukp3", e);
					allDeleted = false;
				}
