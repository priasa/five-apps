...
<div class="wrapper wrapper-content animated fadeIn">
	<div class="ibox float-e-margins">
		<div class="ibox-title">
			<fmt:message key="header.ukme6.listing" />
		</div>
			...
			<form id="filterForm" method="GET" action="?" class="form-inline">
				...
				<display:table id="${id}" name="${rows}" size="${size}" pagesize="20" export="false" class="table table-striped"
					...
					<display:column media="html" title="<input type='checkbox' id='selectall'/><span class='custom-checkbox'></span>" class="selectAll">
						<input name="id" type="checkbox" value="${ukme6.id}" />
						<span class="custom-checkbox"></span>
					</display:column>
					<display:column titleKey="general.code" sortable="true">
						<c:url var="editUrl" value="/web/ukme6/view/${ ukme6.id }">
						</c:url>
						<a href="${ editUrl }"><c:out value="${ ukme6.id }" /></a>
					</display:column>
					<display:column titleKey="puskemas.kode" sortable="true">
						<c:out value="${ ukme6.kodePuskesmas }" />
					</display:column>
					<display:column titleKey="puskemas.name" sortable="true">
						<c:out value="${ ukme6.namaPuskesmas }" />
					</display:column>
					<display:column titleKey="general.year" sortable="true">
						<c:out value="${ ukme6.tahun }" />
					</display:column>
					<display:column titleKey="general.month" sortable="true">
						<c:choose>
						    <c:when test="${ukme6.bulan == 1}">
						       Januari
						    </c:when>
						    <c:when test="${ukme6.bulan == 2}">
						       Februari
						    </c:when>
						    <c:when test="${ukme6.bulan == 3}">
						       Maret
						    </c:when>
						    <c:when test="${ukme6.bulan == 4}">
						       April
						    </c:when>
						    <c:when test="${ukme6.bulan == 5}">
						       Mei
						    </c:when>
						    <c:when test="${ukme6.bulan == 6}">
						       Juni
						    </c:when>
						    <c:when test="${ukme6.bulan == 7}">
						       Juli
						    </c:when>
						    <c:when test="${ukme6.bulan == 8}">
						       Agustus
						    </c:when>
						    <c:when test="${ukme6.bulan == 9}">
						       September
						    </c:when>
						    <c:when test="${ukme6.bulan == 10}">
						       Oktober
						    </c:when>
						    <c:when test="${ukme6.bulan == 11}">
						       November
						    </c:when>
						    <c:when test="${ukme6.bulan == 12}">
						       Desember
						    </c:when>
						    <c:otherwise>
						        NONE.
					...
					<display:column titleKey="general.lastUpdatedDate" sortable="true">
						<c:if test="${ ukme6.submittedDate != null }">
							<fmt:formatDate value="${ ukme6.submittedDate }" pattern="dd/MM/yyyy hh:mm a" />
						</c:if>
					</display:column>
				</display:table>
			</form>
...
function deleteRecords() {
    if ($("[name=id]:checked").length > 0) {
        if (confirm('<fmt:message key="general.delete.confirmation"/>')) {
            ...
            $("#filterForm").attr("action", "${pageContext.request.contextPath}/web/ukme6/delete");
            return true;
        }
    }
    return false;
