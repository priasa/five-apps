		...
		<commons:notification-message/>
			<form id="filterForm" method="GET" action="?" class="form-inline">
				<div class="form-group pull-right"> 
					<input type="text"  placeholder="Tenant Code" id="tenantCode" name="tenantCode" class="input-sm form-control inline" value="${tenantCode}" />
				...
				<display:table id="${id}" name="${rows}" size="${size}" pagesize="20" export="false" class="table table-striped"
					...
					<display:column media="html" title="<input type='checkbox' id='selectall'/><span class='custom-checkbox'></span>" class="selectAll">
						<input name="id" type="checkbox" value="${tenantEntity.id}" />
						<span class="custom-checkbox"></span>
					</display:column>
					<display:column titleKey="general.code" sortable="true">
						<b><c:out value="${ tenantEntity.tenantCode }" /></b>
					</display:column>
					<display:column titleKey="general.name" sortable="true">
						<c:out value="${ tenantEntity.tenantName }" />
					</display:column>
					<display:column titleKey="general.provinsi" sortable="false">
						<c:out value="${ tenantEntity.provinsiName }" />
					</display:column>
					<display:column titleKey="general.kabupaten" sortable="false">
						<c:out value="${ tenantEntity.kabupatenName }" />
					</display:column>
					<display:column titleKey="general.tokenId" sortable="false">
						<b><c:out value="${ tenantEntity.tokenId }" /></b>
					</display:column>
					<display:column titleKey="general.lastUpdatedDate" sortable="true">
						<c:if test="${ tenantEntity.modifiedDate != null }">
							<fmt:formatDate value="${ tenantEntity.modifiedDate }" pattern="dd/MM/yyyy hh:mm a" />
						</c:if>
					</display:column>
				</display:table>
			</form>
